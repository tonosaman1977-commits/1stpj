...
- 出力スキーマ:
  ...
  ```
  ※ 選択されていない言語フィールドは空文字列で返す
- 無料枠: 15 RPM・1,000 RPD

## フロントエンド

- `google.script.run.generateExplanations(text, selectedLanguages)` でGASサーバー呼び出し
  - `selectedLanguages`: 選択された言語コードの配列（例: `["chinese", "korean"]`）
...
カテゴリ別:
- easy_japanese: 短文・難語言い換え・中学生レベル・200〜350文字
- english: シンプルな文構造・過度な専門表現を避ける・120〜200 words
- for_international_students: 研究分野明記・背景説明あり・200〜350文字
- 各外国語: 自然な表現・専門用語は必要に応じて説明・200〜350文字相当
